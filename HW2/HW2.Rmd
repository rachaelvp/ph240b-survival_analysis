...
Because $\Psi(P)$ is a function of $\Psi^1(P)$ we first derive the efficient influence curve for $\Psi^1(P)=\beta=\text{argmin}{\gamma}E_P\big[(Y - (1,A,W)\gamma)^2\big]$. First we need to compute the pathwise derivative (also let's assume that W is $d$-dimensional):
...
$$\frac{d}{d\beta}\text{argmin}{\gamma}E_{P}\big[(Y - (1,A,W)\beta)^2\big] = 0_{d\times 1}\Rightarrow 2\Big[E_{P}\big[(Y - (1,A,W)\beta)(1, A, W)^{\top}\big]\Big] = 0_{d\times 1}\Rightarrow E_{P}\big[(Y - (1,A,W)\beta)(1, A, W)^{\top}\big] = 0_{d\times 1}\ \ \ \ \ \ \ (1)$$ 

And because $\Psi^{1}(P_\epsilon) = \beta_{\epsilon} = \text{argmin}{\gamma}E_{P_\epsilon}\big[(Y - (1,A,W)\gamma)^2\big]$,
$\beta_{\epsilon}$ satisfies 

$$\frac{d}{d\beta_\epsilon}\text{argmin}{\gamma}E_{P_\epsilon}\big[(Y - (1,A,W)\beta_\epsilon)^2 = 0_{d\times 1}\big] \Rightarrow 2\Big[E_{P_\epsilon}\big[(Y - (1,A,W)\beta_\epsilon)(1, A, W)^{\top}\big]\Big] = 0_{d\times 1}\Rightarrow E_{P_\epsilon}\big[(Y - (1,A,W)\beta_\epsilon)(1, A, W)^{\top}\big] = 0_{d\times 1}\ \ \ \ \ \ \ (2)$$ 

From (1) and (2),

$$E_{P}\big[(Y - (1,A,W)\beta)(1, A, W)^{\top}\big] - E_{P_\epsilon}\big[(Y - (1,A,W)\beta_\epsilon)(1, A, W)^{\top}\big] = 0_{d\times 1}$$
$$\Rightarrow E_{P}\big[(Y - (1,A,W)\beta)(1, A, W)^{\top}\big] - E_{P_\epsilon}\big[(Y - (1,A,W)\beta)(1, A, W)^{\top}\big]+ E_{P_\epsilon}\big[(Y - (1,A,W)\beta)(1, A, W)^{\top}\big] - E_{P_\epsilon}\big[(Y - (1,A,W)\beta_\epsilon)(1, A, W)^{\top}\big] = 0_{d \times 1}\text{ .}$$ 
...
This implies 
$$\frac{d \Psi^{'}(P\epsilon)}{d\epsilon}|_{\epsilon = 0}\ = \big[E_{P} (1,A,W)^{\top}(1,A,W)\big]^{-1}E_P\big[(Y-(1,A,W))\beta(1,A,W)^{\top}S(O)\big]=<\big[E_{P} (1,A,W)^{\top}(1,A,W)\big]^{-1}E_P\big[(Y-(1,A,W))\beta(1,A,W)^{\top}\big],S(O)> { .}$$
